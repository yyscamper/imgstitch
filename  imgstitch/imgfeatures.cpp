...
int export_lowe_features_1( char*, struct feature*, int );
...
int export_lowe_features( char* filename, struct feature* feat, int n)
{
	...
	for( i = 0; i < n; i++ )
	{
		fprintf( file, "%d %d %f %d", cvFloor(feat[i].x), cvFloor(feat[i].y),
				feat[i].ori,cvRound(feat[i].scl));
		for( j = 0; j < d; j++ )
		{
